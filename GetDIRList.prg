...
FUNC GDL_FFList( ;                        // Filtered File List  
    ...
    *
    IF nIncludes > 1 .OR. lChangeDir
       ...
       AEVAL( aTemp, { | a1 | IF( "D" $ a1[ 5 ], AADD( aRVal, a1 ), ) } ) 
    ENDIF lInclDIRs 
    
    IF nIncludes # 2
       FOR nFilter := 1 TO LEN( aFilter ) 
          aTemp := DIRECTORY( cPath + aFilter[ nFilter ], cAttrib  )
          FOR nEntry := 1 TO LEN ( aTemp ) 
             c1Entry := aTemp[ nEntry, 1 ]
             IF ASCAN( aRVal, { | a1 | c1Entry == a1[ 1 ] } ) < 1
                AADD( aRVal, aTemp[ nEntry ] )
             ENDIF   
          NEXT nEntry
       NEXT nFilter 
    ENDIF nIncludes # 2
