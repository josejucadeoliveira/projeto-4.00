...
DEFINE WINDOW CANCELA_NFe;
       ...
       WIDTH 600 ;
       HEIGHT 450;
       TITLE 'CANCELA_NFe  MODELO 65 ' ICON "ICONE01";
       MODAL;
       NOSIZE;
 ...
 @ 05,80 textBTN txt_DAV;
    ...
    DEFINE LABEL Lbl_Serie
	 ...
	 DEFINE TEXTBOX Txt_JUSTI
            ROW  100
            COL   10
            WIDTH  500
            HEIGHT 30
            VALUE  "VENDE NAO CONCRETIZADO OU SUBTISUICAO POR NFE "
            MAXLENGTH 200
            UPPERCASE  .T.
	        ON GOTFOCUS This.BackColor:=clrBack 
   	        ON LOSTFOCUS This.BackColor:=clrNormal  
...
DEFINE BUTTON Button_42332
   ...
   @ 200, 35 LISTBOX ListBox_2 ;
   WIDTH 200+nLarguraTela
   HEIGHT 150 ;
...
proc cas_atualizador_cancelamento(achave)
local nn :=  CANCELA_NFe.ListBox_2.ItemCount + 1
 CANCELA_NFe.ListBox_2.AddItem( achave )
 CANCELA_NFe.ListBox_2.value := nn
return
...
pdf:=alltrim(zNUMERO+xxANO+"-pdf")
  ...
  PdfbDir := DiskName()+":\"+CurDir()+"\"+pdf+"\"
    ...
    ENDIF
Reconectar_A() 
  oQuery:=oServer:Query( "SELECT CHAVE,MSCANCELAMENTO, nt_retorno ,AUTORIZACAO  FROM nfce WHERE CbdNtfNumero = "+AllTrim(pACode)+" and CbdNtfSerie ="+(paserie)+" Order By CbdNtfNumero" )
 If oQuery:NetErr()
  ...
  Endif
    ...
    Cbdcheveevento:=alltrim(oRow:fieldGet(2))
      *  cXML_NOTA   :=Memoread(oRow:fieldGet(3))
...
xJust :=CANCELA_NFe.Txt_JUSTI.value
...
IF   GERA_NFE_NFCE=1
cRet := MON_ENV("NFe.CancelarNFe("+pchave+","+xJust+","+cCNPJ+","+nEvento+")")
MY_WAIT( 1 ) 
...
//////////////////enviar/////////////////////////
 cRet       := MON_ENV("NFE.ConsultarNFe("+ALLTRIM(nChNFe)+")")
  MY_WAIT( 1 ) 
       lRetStatus:=EsperaResposta(cDestino)
          if lRetStatus==.t.
        if SUBSTR(memoread(cDestino), 1, 4)=="ERRO"
   ...
   BEGIN INI FILE "C:\ACBrMonitorPLUS\SAI.TXT"
	   ...
	   get nChNFe          section  "CONSULTA"       ENTRY "ChNFe"
...
vNFE        := substr(nChNFe, 26, 09)
cSerie      :=val(SUBSTR(nChNFe,23,3))
nnfe:="NFE"+ALLTRIM(vNFE)
MY_WAIT( 1 ) 


fxml:="C:\ACBrMonitorPLUS\"+nChNFe+"-nfe.xml"
ffxml    :=memoread(fxml)
MY_WAIT( 1 ) 
...
if RRCStat="135"
 ...
 cQuery := "UPDATE NFCE SET  MSCANCELAMENTO = '"+c_XMotivo+"',nfcedaruma= '"+ffxml+"',Cbdcheveevento ='"+alltrim(cPesqEVENT)+"' WHERE CbdNtfNumero = "+(alltrim(paCode))+" AND cbdmod= "+"65"+" and CbdNtfSerie = "+paserie+" "
 oQuery:=oServer:Query( cQuery )
  If oQuery:NetErr()
  	MsgStop(oQuery:Error())
    MsgInfo("Por Favor Selecione o registro SS 408 PROPLEMA")
    Return Nil
  Endif
 MY_WAIT( 1 ) 
///////////////////pega a qtd nos itens 
 oQuery :=oServer:Query( "SELECT CbdcProd,CbdqCOM,CbdcEAN FROM ITEMNFCE WHERE CbdNtfNumero = "+AllTrim(paCode)+" and CbdNtfSerie = "+paserie+"  Order By CbdNtfNumero" )
If oQuery:NetErr()												
  MsgStop(oQuery:Error())
...
///////////////////pega a qtd no estoque
  ...
  HB_Cria_Log_cancela(xCODBAR , +"  Quantidade em Estoque             " ,+  ntrim(Xqtd))
...
If aQuery:NetErr()												
 ...
 MsgInfo("Por Favor Selecione o registro LINHA 302 ") 
   ...
   HB_Cria_Log_cancela(xCODBAR        , +"Quantidade Atualizado no DB          " ,+  ntrim( RQTD))
   HB_Cria_Log_cancela((paCode)+"     itens  Cancelado "+ntrim(nn),+" **************************************************************************" ,+  "")
...
///////////////////pega a qtd nos itens 
 oQuery :=oServer:Query( "SELECT CbdcProd,CbdqCOM FROM ITEMNFCE WHERE CbdNtfNumero = "+AllTrim(paCode)+" and CbdNtfSerie = "+paserie+"  Order By CbdNtfNumero" )
If oQuery:NetErr()												
  MsgStop(oQuery:Error())
 MsgInfo("Por Favor Selecione o registro deu zebra ") 
Endif
...
*if c_CStat="135"
c_XMotivo:='Evento registrado e vinculado a NF-e'
cQuery := "UPDATE NFcE SET MSCANCELAMENTO  = '"+c_XMotivo+"',RETORNO_EVENTO= '"+(AllTrim(ffxml))+"',Cbdcheveevento ='"+alltrim(cPesqEVENT)+"' WHERE CbdNtfNumero = "+(alltrim(paCode))+" AND cbdmod= "+"65"+" and CbdNtfSerie = "+paserie+" "
 oQuery:=oServer:Query( cQuery )
  If oQuery:NetErr()
  	MsgStop(oQuery:Error())
    MsgInfo("Por Favor Selecione o registro SS 4732 PROPLEMA")
...
vdata:=dtos(date()-120)
vdata1:=dtos(date())
TQuery :=oServer:Query( "SELECT CbdcProd,CbdqCOM FROM ITEMNFCE WHERE (CbddEmi >= "+vdata+" and CbddEmi <= "+vdata1+" ) Order By CbddEmi" )
