...
Procedure ler_arquivo( lStartUp )
	...
	DEFINE WINDOW Form_1 				;
		...
		DEFINE NOTIFY MENU 
			ITEM 'Auto&Run'		ACTION ( lWinRun := !lWinRun, ;
				...
				NAME Auto_Run
			SEPARATOR	
			ITEM 'Open &Notes'	ACTION RunHotFile()
			ITEM '&Options'		ACTION SetOptions()
			ITEM '&Help'		ACTION _Execute( _HMG_MainHandle, "open", 'HotFile.hlp' )
			ITEM 'A&bout...'	ACTION ShellAbout( "", PROGRAM + VERSION + CRLF + ;
				"Copyright " + Chr(169) + COPYRIGHT, LoadMainIcon(GetInstance(), "MAIN") )
			SEPARATOR	
...
*--------------------------------------------------------*
  ...
  DEFINE WINDOW Form_2 ; 
    ...
    ON PAINT ( hIcon := LoadTrayIcon( GetInstance(), "MAIN" ), ;
    ...
    @ 107,334 BUTTON CONTROL_10 ; 
        CAPTION '&Help' ; 
        ACTION { || _Execute( _HMG_MainHandle, "open", 'HotFile.hlp' ) } ; 
        WIDTH 80 ; 
        HEIGHT 23 ; 
