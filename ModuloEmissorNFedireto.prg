...
*************************************************************
*nfe20 
*CbdvDesc_ttlnfe=desconto em valor 
*CbdvOutro      =desconto em %
*CbdvDesc_cob   =desconto em valor

*itens itemnfe
*CbdvDesc       = xdescontovalor desconto em valor
*cbdvoutro_item  =xdescontoproc   desconto em %
****************************************************************
/*
...
DEFINE TAB Tab_Movimento AT 180,00;
 ...
 PAGE "Produtos" 
	...
	@ 20,05 GRID fita ;
		WIDTH 1000 ;
		HEIGHT 300 ;
	    WIDTHS {50,110,380,80,50,90,100,100,80,60,80 };
        HEADERS {'Itens','Codigo','Descrição','Ncm','Und.','Qtd','Valor R$','Desc.R$','Sub-Total R$','%Icms','CST' };
        VALUE 1 ;			
...
ITEMNFE->(dbgotop())
       while .not. eof()
       ADD ITEM {STR(ITEMNFE->ITENS,3),ITEMNFE->PRODUTO,ITEMNFE->descricao,ITEMNFE->ncm,ITEMNFE->unid,transform((ITEMNFE->qtd),"999,999.99"),transform(ITEMNFE->VALOR, '9,999,999.99'),transform(ITEMNFE->valor_Desc,"99,999.9999"),transform((ITEMNFE->VALOR*ITEMNFE->qtd),"999,999.99"),transform(ITEMNFE->aliquota,"999.99"),transform(ITEMNFE->CST, '999')}TO FITA OF NFe
        ITEMNFE->(dbskip())
       end
select ITEMNFE
SUM SUBTOTAL TO nTtFactura 
...
//IF nfe.Rdg_TIPO1.VALUE == 1
// NFE.Edit_Aplicacao.VALUE:='NOTA FISCAL ELETRONICA REFERENTE AO CUPOM FISCAL'+"    "+"EMITIDO  PELA  A  ECF    "+DADOSNFE->SERIE  +"   COO  "+STRZERO(DADOSNFE->num_seq,6)+"    "+"CCF   "+ (DADOSNFE->CCF) ; 
//+CRLF+"Valor crédito do ICMS        "     +   TOTALICMS       + "          aliquota De      " +  NTOTAL  +  "   pode ser aproveitado nos termos do art. 23 da LC 123 (Simples Nacional)"
//else
 //NFE.Edit_Aplicacao.VALUE:="Valor crédito do ICMS        "     +   TOTALICMS       + "          aliquota De      " +  NTOTAL  + "%"+ "   pode ser aproveitado nos termos do art. 23 da LC 123 (Simples Nacional)"
//endif
...
IF nfe.oRad3.VALUE == 1
*DQuery := oServer:Query( "Select NUM_SEQ, NRPED, cupom, data_orc, cod_cli, nom_cli, cl_cgc ,rgie, cl_end, cl_pessoa, cl_cid, cod_ibge, ed_numero, email, cep, bairro, estado, desconto, ccf, serie, total_ven, valor_tot, desc1, desc2 From cupom WHERE NUM_SEQ = "+chave+" Order By emissao" )
DQuery := oServer:Query( "Select CbdNtfNumero,CbdvProd_ttlnfe,CbdvNF,CbddEmi,CbdvDesc_cob,CbdvOutro,CbdvDesc_ttlnfe From NFE20 WHERE CbdNtfNumero = "+chave+" Order By CbddEmi" )
...
DQuery := oServer:Query( "Select NUM_SEQ, NRPED, cupom, data_orc, cod_cli, nom_cli, cl_cgc ,rgie, cl_end, cl_pessoa, cl_cid, cod_ibge, ed_numero, email, cep, bairro, estado, desconto, total_ven, valor_tot, desc1, desc2 From ORCAMENT WHERE NUM_SEQ = "+chave+" Order By emissao" )
If DQuery:NetErr()
  	MsgStop(DQuery:Error())
...
if nfe.oRad3.VALUE == 3 
         ...
         if empty(NFe.txt_DAV.value)
	           msgexclamation('Digite o orcamento nao encontrado','Atenção')
               nfe.txt_dav.SetFocus
               nfe.txt_dav.VALUE:=0	  
               return(.f.)
         endif

DQuery := oServer:Query( "Select NUM_SEQ, NRPED, cupom, data_orc, cod_cli, nom_cli, cl_cgc ,rgie, cl_end, cl_pessoa, cl_cid, cod_ibge, ed_numero, email, cep, bairro, estado, desconto, total_ven, valor_tot, desc1, desc2 From DEVOLUCAO WHERE NUM_SEQ = "+chave+" Order By emissao" )
If DQuery:NetErr()
  	MsgStop(DQuery:Error())
   MsGInfo("linha 1740  " + oServer:Error() )
    Return Nil
...
*cQuery:= "select uf,cnpj,ie,raz_social,endereco,numero,bairro,cod_ibge,CIDADE ,CEP FROM forneced WHERE codigo	= " + AllTrim(pCode)         
...
IF nfe.oRad3.VALUE == 1
eQuery := oServer:Query( "Select CbdcProd ,CbdcEAN,CbdxProd,CbdnItem,CbdvProd,CbdNtfNumero,CbdNCM,CbdqCOM,CbdvUnCom,CbdUCom,CbdvDesc,cbdvoutro_item,CbdvAliq From nfeitem WHERE CbdNtfNumero ="+ntrim(xnum)+" Order By CbdxProd" )
endif
IF nfe.oRad3.VALUE == 2
 eQuery := oServer:Query( "Select COD_PROD,produto,descricao,itens,subtotal,nseq_orc,ncm,qtd,valor,unid,unid From ITEMORC WHERE nseq_orc ="+ntrim(xnum)+" Order By descricao" )
endif
IF nfe.oRad3.VALUE == 3
 eQuery := oServer:Query( "Select COD_PROD,produto,descricao,itens,SUBTOTAL,nseq_orc,ncm,qtd,valor,unid,unid From ITEMDEVOLUCAO WHERE nseq_orc ="+ntrim(xnum)+" Order By descricao" )
endif
...
IF  xestado="RO" .AND. xALIQUOTA<0
nfe.textBTN_cfop.value :=5102
ELSE 
nfe.textBTN_cfop.value :=5403
ENDIF

IF  xestado<>"RO" .AND. xALIQUOTA<0
nfe.textBTN_cfop.value :=6102
ELSE
nfe.textBTN_cfop.value :=6403
ENDIF
*ENDIF
*endif
...
xVALOR_TOT:=int(xVALOR_TOT*100)/100
 ...
 NFE.Txt_valortotal.value   :=xTOTAL_VEN // XVALOR
 NFE.Txt_total.value        :=xVALOR_TOT  //XSUBTOTAL
 NFE.Txt_desconto.value     :=xdesc2      //XVALOR_DESC
 NFE.Txt_desconto1.value    :=PORCDESCONTO
 NFE.txt_VALOR.value        :=xTOTAL_VEN //XSUBTOTAL
...
LIN_21:="Valor Aprox dos Tributos R$"+" "+cDescricao
 ...
 COMPLEMENTO:="Valor crédito do ICMS        "     +   TOTALICMS       + "          aliquota De      " +  NTOTAL  +  "   pode ser aproveitado nos termos do art. 23 da LC 123 (Simples Nacional)" +"      "+LIN_21 
   
// aadd(Obs_Nota,{0,'Documento emitido por empresa optante pelo Simples Nacional, permite aproveitamento de '+;
//			                 'credito de ICMS no Valor de R$ '+transf(TOTALICMS,'999,999.99')+', correspondente a Aliquota de '+;
//							 transf(NTOTAL,'999.99')+'% nos termos do art. 23 da LC 123'  })
...
*Serie=1
*nNF=3011
xrefNFe:="35100200128791000130550010000030110000030118"		 
xcUF:="35"
		
		 	 
		 aadd(DADOS_NFe,{'[NFRef001]'})
		 aadd(DADOS_NFe,{'refNFe='+xrefNFe })
		 aadd(DADOS_NFe,{'cUF='+xcUF})
		 aadd(DADOS_NFe,{'nNF='+'3011'})
		 aadd(DADOS_NFe,{'AAMM='+"1411"})
		 aadd(DADOS_NFe,{'cnpj='+"84712611000152"})
...
reg:=1
	 	 ...
	 	 aadd(DADOS_NFe,{'[NFRef'+strzero(reg,3)+']' })
...
mgREGIME:=1
        ...
        nFrete_Item:=0
		 ...
		 aadd(DADOS_NFe,{'ValorTotal=' +ALLTRIM(TRANSFORM(ITEMNFE->SubTotal,"@ 999999999999.999")) })
         aadd(DADOS_NFe,{'ValorDesconto=' +ALLTRIM(TRANSFORM(ITEMNFE->SubTotal*DADOSNFE->DESC1/100,"@ 99999999999999.99999"))  })
		* aadd(DADOS_NFe,{'ValorDesconto=' +ALLTRIM(TRANSFORM(ITEMNFE->Valor_DESC,"@ 99999999999999.999"))  })
		 aadd(DADOS_NFe,{'vFrete='+ALLTRIM(TRANSFORM((nFrete_Item),"@! 999999999999.99")) })
		 aadd(DADOS_NFe,{'NumeroDI=1' })
	 	 aadd(DADOS_NFe,{'vTotTrib='+ALLTRIM(TRANSFORM(ITEMNFE->N_IBPT  ,"99,999,999.999")) })
...
VV_IBPT:=VV_IBPT+ITEMNFE->N_IBPT
...
//totis da nfe   
    ...
    oQuery	:= oServer:Query( "UPDATE nfe20 SET cbdcrt='"+alltrim(str(ccbdcrt))+"',CbdCPF_dest  = '"+ClienteTxtCGC+"' , CbdIE_dest = '"+Insc+"',CbdxNome_dest = '"+cCbdxNome_dest+"' , CbdxLgr_dest ='"+cCbdxLgr_dest+"',Cbdnro_dest='"+CCbdnro_dest+"',CbdxBairro_dest='"+CCbdxBairro_dest+"' ,CbdxEmail_dest = '"+CCbdxEmail_dest+"',CbdxCpl_dest='"+CCbdxCpl_dest+"',CbdcMun_dest='"+CCbdcMun_dest+"',CbdxMun_dest='"+CCbdxMun_dest+"' ,CbdUF_dest='"+cCbdUF_dest+"',CbdCEP_dest='"+CCbdCEP_dest+"',CbdcPais_dest='"+cCbdcPais_dest+"',CbdxPais_dest='"+CCbdxPais_dest+"' ,Cbdfone_dest='"+CCbdfone_dest+"',CbdISUF='"+CCbdISUF+"',CbdvBC_ttlnfe='"+alltrim(str(cCbdvBC_ttlnfe))+"',CbdvICMS_ttlnfe='"+alltrim(str(cCbdvICMS_ttlnfe))+"',CbdvBCST_ttlnfe='"+alltrim(str(cCbdvBCST_ttlnfe))+"',CbdvFrete_ttlnfe='"+alltrim(str(cCbdvFrete_ttlnfe))+"',CbdmodFrete ='"+alltrim(str(cCbdmodFrete))+"',CbdvST_ttlnfe='"+alltrim(str(cCbdvST_ttlnfe))+"',CbdvProd_ttlnfe='"+alltrim(str(cCbdvProd_ttlnfe))+"',CbdvSeg_ttlnfe ='"+alltrim(str(cCbdvSeg_ttlnfe ))+"',CbdvDesc_ttlnfe='"+alltrim(str(cCbdvDesc_ttlnfe))+"',CbdvII_ttlnfe='"+alltrim(str(cCbdvII_ttlnfe))+"',CbdvIPI_ttlnfe='"+alltrim(str(cCbdvIPI_ttlnfe))+"',CbdvPIS_ttlnfe='"+alltrim(str(cCbdvPIS_ttlnfe))+"',CbdvCOFINS_ttlnfe='"+alltrim(str(cCbdvCOFINS_ttlnfe))+"',CbdvOutro='"+alltrim(str(cCbdvOutro))+"',CbdvNF='"+alltrim(str(cCbdvNF))+"',CbdEmailArquivos='"+alltrim(str(cCbdEmailArquivos))+"',CbdTitGenerico='"+alltrim((cCbdTitGenerico))+"',CbdTxtGenerico='"+alltrim(cCbdTxtGenerico)+"'  WHERE CbdNtfNumero = " +(C_CbdNtfNumero) )
 	If oQuery:NetErr()												
     	MsgStop(oQuery:Error())
...
/*
   ...
   cQuery:="INSERT INTO tb_auxiliar (ax_tipo,ax_codigo,ax_descricao,tipo_doc,ax_auxiliar1, ax_auxiliar2 ) VALUES ("
            cQuery+=ClipValueSQL2("PR")                                  +" ,"
            cQuery+=ClipValueSQL2(xCbdNtfNumero)                               +" ,"
            cQuery+=ClipValueSQL2(UPPER(CCbdxNome_dest))            +" ,"
		    cQuery+=ClipValueSQL2(UPPER("XML"))            +" ,"
            cQuery+=ClipValueSQL2(Limpa_Campo(xCbdNtfNumero))        +" ,"
            cQuery+=ClipValueSQL2(xCbdNtfNumero)                                  +" )"
...
//MSGINFO(PATH+"\NOTA.TXT")
//XML:=ALLTRIM(XML)
			    // Inclui se ainda não existir
         cQuery:="INSERT INTO tb_documento (dc_codigo, dc_anexo, dc_sequencia,nt_retorno) VALUES ("
         cQuery+=ClipValueSQL2(Limpa_Campo(xCbdNtfNumero))    +" ,"
         cQuery+=ClipValueSQL2(AllTrim(ffxml))                 +" ,"
		 cQuery+=ClipValueSQL2(xCbdNtfNumero)                 +" ,"
         cQuery+=ClipValueSQL2(AllTrim(ffxml))                 +" )"
         oQuery := oServer:Query(cQuery )
         If oQuery:NetErr()
...
registro:=registro+1
	 ...
	 xCbdvDesc		    := ITEMNFE->Valor_DESC
   	 ...
   	 xcbdindtot         := 1
      cQuery :="INSERT INTO nfeitem (CbdEmpCodigo, CbdNtfNumero,CbdNtfSerie, CbdnItem ,CbdcProd ,CbdxProd,CbdNCM ,CbdEXTIPI,Cbdgenero, CbdCFOP, CbduCOM,CbdqCOM,CbdvUnCom ,CbdvProd,CbduTrib,CbdqTrib,CbdvUnTrib,CbdnTipoItem, CbdvDesc, cbdindtot ) VALUES ( '"+alltrim(str(xCbdEmpCodigo ))+"' , '"+alltrim(str(xCbdNtfNumero))+ "', '"+alltrim(str(xCbdNtfSerie))+ "', '"+alltrim(str(xCbdnItem))+ "', '"+alltrim((xCbdcProd))+ "', '"+alltrim(xCbdxProd)+"','"+xCbdNCM+"','"+xCbdEXTIPI+"','"+alltrim(str(xCbdgenero))+"','"+alltrim(str(xCbdCFOP))+"','"+ xCbduCOM+"','"+alltrim(str(xCbdqCOM))+"','"+alltrim(str(xCbdvUnCom))+"','"+alltrim(str(xCbdvProd))+"','"+alltrim(xCbduTrib)+"','"+alltrim(str(xCbdqTrib))+"','"+alltrim(str(xCbdvUnTrib))+"','"+alltrim(str(xCbdnTipoItem))+"','"+alltrim(str( xCbdvDesc))+"','"+alltrim(str(xcbdindtot))+"' ) "
		oQuery	:= oServer:Query( cQuery )
	If oQuery:NetErr()												
      	MsgStop(cQuery:Error())
