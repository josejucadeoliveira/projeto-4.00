...
* Impostos_Cupom  :=Impostos_Cupom+Impostos_Cupom_1
 ...
 NA_IBPT         :=ITEMNFCE->SUBTOTAL*aliqnac/100   
 MP_IBPT         :=ITEMNFCE->SUBTOTAL*aliqimp/100   
 ES_IBPT         :=ITEMNFCE->SUBTOTAL*aliqest/100   
...
mgREGIME:=1
        ...
        nFrete_Item:=0
		 ...
		 aadd(DADOS_NFe,{'NumeroDI=1' })
	 	 aadd(DADOS_NFe,{'vTotTrib='+ALLTRIM(TRANSFORM(ITEMNFE->N_IBPT+ITEMNFE->m_IBPT+ITEMNFE->e_IBPT  ,"99,999,999.999")) })
...
registro:=registro+1
	 ...
	 xcbdxped_item      :=ntrim(ITEMNFE->SEQ_T)
   	 xcbdindtot         := 1
      cQuery :="INSERT INTO nfeitem (CbdcEAN,cbdcsittrib,CbdvAliq,CbdEmpCodigo, CbdNtfNumero,CbdNtfSerie, CbdnItem ,CbdcProd ,CbdxProd,CbdNCM ,CbdEXTIPI,Cbdgenero, CbdCFOP, CbduCOM,CbdqCOM,CbdvUnCom ,CbdvProd,CbduTrib,CbdqTrib,CbdvUnTrib,CbdnTipoItem, CbdvDesc, cbdindtot,cbdxped_item ) VALUES ('"+xCbdcEAN+"', '"+Xcbdcsittrib+"','"+XCbdvAliq+"' ,'"+alltrim(str(xCbdEmpCodigo ))+"' , '"+alltrim(str(xCbdNtfNumero))+ "', '"+alltrim(str(xCbdNtfSerie))+ "', '"+alltrim(str(xCbdnItem))+ "', '"+alltrim((xCbdcProd))+ "', '"+alltrim(xCbdxProd)+"','"+xCbdNCM+"','"+xCbdEXTIPI+"','"+alltrim(str(xCbdgenero))+"','"+alltrim(str(xCbdCFOP))+"','"+ xCbduCOM+"','"+alltrim(str(xCbdqCOM))+"','"+alltrim(str(xCbdvUnCom))+"','"+alltrim(str(xCbdvProd))+"','"+alltrim(xCbduTrib)+"','"+alltrim(str(xCbdqTrib))+"','"+alltrim(str(xCbdvUnTrib))+"','"+alltrim(str(xCbdnTipoItem))+"','"+alltrim(str( xCbdvDesc))+"','"+alltrim(str(xcbdindtot))+"','"+xcbdxped_item+"') "
		oQuery	:= oServer:Query( cQuery )
	If oQuery:NetErr()												
      	MsgStop(cQuery:Error())
