...
@ 580,520 BUTTONEX OButton_18 ;
 ...
 @ 630,520 BUTTONEX OButton_11 ;
  ...
  DEFINE GRID Grid_notas
			...
			WIDTH  AJANELA-10
            HEIGHT 200
            WIDTHS {80,50,250,150,100,100,100,350,200,200,400,400,300}
		    HEADERS {"N Nf-e","Serie","Consumidor","Cnpj/Cpf","data","Horas","Valor","Chave","Autorização","XML","Evento","NOTATXT","nfcedaruma"}
...
oQuery:= oServer:Query( "Select CbdNtfNumero,CbdNtfSerie,CbdxNome_dest,CbdCNPJ_dest,CbddEmi,horas,cbdVNF,Chave,autorizacao,nt_retorno,MSCANCELAMENTO,ARQUIVO_TXT,RETORNO_EVENTO From NFE20 WHERE CbdxNome_dest like  "+cSearch+" Order By CbddEmi" )
...
If !Empty(oRow:fieldGet(3)) // se nao encontra vale a pesq pro nota fiscal
else
c_barras:=CHARREM(janelanfe.cSearch.Value)
c_barras:=CHARREM(CHAR_REMOVE,janelanfe.cSearch.Value )
c_barras:=val(c_barras)
c_barras:=alltrim(str(c_barras))
C_barras:= ' "'+Upper(AllTrim(c_barras))+'%" '

oQuery:= oServer:Query( "Select CbdNtfNumero,CbdNtfSerie,CbdxNome_dest,CbdCNPJ_dest,CbddEmi,horas,cbdVNF,Chave,autorizacao,nt_retorno,MSCANCELAMENTO,ARQUIVO_TXT,RETORNO_EVENTO From NFE20 WHERE CbdNtfNumero like  "+C_barras+" Order By CbddEmi" )
janelanfe.cSearch.setfocus
...
oQuery := oServer:Query( "Select CbdNtfNumero,CbdNtfSerie,CbdxNome_dest,CbdCNPJ_dest,CbddEmi,horas,cbdVNF,Chave,autorizacao,nt_retorno MSCANCELAMENTO,NOTATXT,RETORNO_EVENTO From NFE20 WHERE CbddEmi="+vdata+" and horas  like "+choras+"  Order By CbddEmi")
If oQuery:NetErr()												
  MsgInfo("ERROR NO SEVIDOR MYSQL " + oQuery:Error())
 Return Nil
Endif
...
oQuery := oServer:Query( "Select CbdNtfNumero,CbdNtfSerie,CbdxNome_dest,CbdCNPJ_dest,CbddEmi,horaS,cbdVNF,Chave,autorizacao,nt_retorno,MSCANCELAMENTO,NOTATXT,RETORNO_EVENTO From NFE20 WHERE CbddEmi ="+dtos(date())+" Order By HORAS" )
...
IF NFE_EMITIDA_CONTIGENCIA.RdG_Ambiente_NFCe.VALUE == 1
 ...
 oQuery := oServer:Query( "Select CbdNtfNumero,CbdNtfSerie,CbdxNome_dest,CbdCNPJ_dest,CbddEmi,HORAS,cbdVNF,Chave,autorizacao,nt_retorno,MSCANCELAMENTO,NOTATXT,ARQUIVO_TXT From NFE20 WHERE CbddEmi >= "+vdata+" and CbddEmi <= "+vdata1+" Order By CbddEmi" )
ELSE
  vdata:=dtos(NFE_EMITIDA_CONTIGENCIA.dpi_001.value)
  vdata1:=dtos(NFE_EMITIDA_CONTIGENCIA.dpi_002.value)
  oQuery := oServer:Query( "Select CbdNtfNumero,CbdNtfSerie,CbdxNome_dest,CbdCNPJ_dest,CbddEmi,HORAS,cbdVNF,Chave,autorizacao,nt_retorno,MSCANCELAMENTO,NOTATXT,ARQUIVO_TXT From NFE20 WHERE CbddEmi >= "+vdata+" and CbddEmi <= "+vdata1+" and nt_retorno=''  Order By CbddEmi" )
ENDIF
...
REG:=0
NTOTAL=0
oQuery := oServer:Query( "Select CbdNtfNumero,CbdcProd,CbdxProd,CbdNCM ,CbdCFOP,CbdqCOM,CbdvUnCom,CbdvProd From itemnf WHERE CbdNtfNumero LIKE "+pcode+" and CbdNtfSerie = "+alltrim(Txt_SERIE)+" Order By CbdxProd" )
For i := 1 To oQuery:LastRec()
  oRow := oQuery:GetRow(i)
  REG=REG+1
...
Local notatxt := (AllTrim((GetColValue( "Grid_notas", "janelanfe", 12 ))))
...
oQuery        :=oServer:Query( "SELECT CHAVE,nt_retorno,ARQUIVO_TXT FROM NFE20 WHERE CbdNtfNumero = "+ntrim(pnumero)+" AND  CbdNtfSerie = "+pserie+"  Order By CbdNtfNumero" )
oRow          := oQuery:GetRow(1)
...
IF c_CStat == "101"
cXml     :=Memoread(fxml)
 cQuery := "UPDATE NFE20 SET nt_retorno='"+alltrim(ffxml)+"' WHERE CbdNtfNumero = "+((vNFE))+" AND cbdmod= "+"65"+" and CbdNtfSerie = "+ntrim(cSerie)+" "
...
cQuery := "UPDATE NFE20 SET MSCANCELAMENTO  = '"+c_XMotivo+"', AUTORIZACAO='"+"INUTILIZADO"+"',nt_retorno='"+cXml+"' WHERE CbdNtfNumero = "+ntrim(pnumero)+" AND cbdmod= "+"55"+" and CbdNtfSerie = "+(pserie)+" "
*cQuery := "UPDATE NFCE SET MSCANCELAMENTO  = '"+c_XMotivo+"' ,AUTORIZACAO='"+"INUTILIZADO"+"',nt_retorno='"+cXml+"' WHERE CbdNtfNumero = "+nIni+" AND cbdmod= "+"55"+" and CbdNtfSerie = "+nSer+" "
...
public c_cNProt   :=cNProt
AUTO              :=c_cNProt
 cQuery := "UPDATE NFE20 SET CHAVE='"+cNovaChave+"' WHERE CbdNtfNumero = "+((vNFE))+" AND cbdmod= "+"55"+" and CbdNtfSerie = "+ntrim(cSerie)+" "
 oQuery:=oServer:Query( cQuery )
  If oQuery:NetErr()
  	MsgStop(oQuery:Error())
    MsgInfo("Por Favor Selecione o registro SS 4732 PROPLEMA")
...
ffxml    :=memoread(fxml)
cXml     :=Memoread(fxml)
 cQuery := "UPDATE NFE20 SET AUTORIZACAO='"+c_cNProt+"' ,nt_retorno='"+alltrim(ffxml)+"' WHERE CbdNtfNumero = "+((vNFE))+" AND cbdmod= "+"55"+" and CbdNtfSerie = "+ntrim(cSerie)+" "
 oQuery:=oServer:Query( cQuery )
  If oQuery:NetErr()
  	MsgStop(oQuery:Error())
    MsgInfo("Por Favor Selecione o registro SS 4732 PROPLEMA")
...
xANO     :=ALLTRIM(SUBSTR(XANO,0,6))
  ...
  oQuery:=oServer:Query( "SELECT CHAVE,nt_retorno,ARQUIVO_TXT,NOTATXT  FROM nfe20 WHERE CbdNtfNumero = "+AllTrim(pACode)+" and CbdNtfSerie ="+(paserie)+" Order By CbdNtfNumero" )
 If oQuery:NetErr()
...
*--------------------------------------------------------------*
Local paCode       :=(GetColValue( "Grid_notas", "janelanfe", 1 ))
Local Serie_nfe    :=(GetColValue( "Grid_notas", "janelanfe", 2 ))
Local xnome      :=(GetColValue( "Grid_notas", "janelanfe", 3 ))
...
Local xCbdNtfSerie  := (GetColValue( "Grid_notas", "janelanfe", 2 ))
Local paCode        :=  (GetColValue( "Grid_notas", "janelanfe", 1 ))
...
*--------------------------------------------------------------*
local pnumero:= (val((GetColValue( "Grid_notas", "janelanfe", 1 ))))
Local pTEXTE:= (AllTrim((GetColValue( "Grid_notas", "janelanfe", 12 ))))
...
oQuery        :=oServer:Query( "SELECT CHAVE,nt_retorno,ARQUIVO_TXT FROM NFE20 WHERE CbdNtfNumero = "+NTRIM(pnumero)+" AND  cbdmod= "+"65"+" and CbdNtfSerie = "+pserie+"  Order By CbdNtfNumero" )
oRow          := oQuery:GetRow(1)
xchave        :=ALLTRIM(oRow:FieldGet(1))
...
serie_i :=NFE_EMITIDA_CONTIGENCIA.serie_i.value
  ...
  oQuery := oServer:Query( "Select CbdNtfNumero,CbdNtfSerie,CbdxNome_dest,CbdCNPJ_dest,CbddEmi,HORAS,cbdVNF,Chave,autorizacao,nt_retorno,Cbdcheveevento,NOTATXT,RETORNO_EVENTO From NFE20 WHERE CbdNtfNumero >= "+vnumero+" and CbdNtfNumero <= "+vnumero1+" and CbdNtfSerie="+serie_i+" Order By CbdNtfNumero" )
