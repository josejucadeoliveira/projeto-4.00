...
///////////////////////////////////////////
 ...
 @ 25,410 textBTN txt_DAV;
                   ...
                   maxlength 40;
		           rightalign;
                   ON enter {||PESQ_PVENDA_2(),peganfe()}
  ...
  */
  ...
  END PAGE
			 ...
			 ON GOTFOCUS SetControl(.T.) ON LOSTFOCUS setControl(.F.) NOTABSTOP
                ...
                @ 650,136 BUTTON   btExcluir           CAPTION "Exclui Itens"      FONT "Cambria" SIZE 12 BOLD WIDTH 125 HEIGHT 25 FLAT ACTION  wnexcluiitem()
...
Function ProcedureescreverINI_3()
...
referencia:= 'Nota Fiscal Referenciada  Chave da NFC-e: =    '      +XNFE_REFERENCIADA  +;
+";emitida anteriormente e vinculada a NF-e atual       "+'NUMERO        ' +nfe.txt_nfecc.value 
else
referencia:=""
endif
...
DQuery := oServer:Query( "Select CbdNtfNumero,CbdvProd_ttlnfe,CbdvNF,CbddEmi,CbdvDesc_cob,CbdvOutro,CbdvDesc_ttlnfe,chave From NFCE WHERE CbdNtfNumero = "+chave+" AND cbdmod= "+"65"+" and CbdNtfSerie = "+(XSERIE)+"  Order By CbddEmi" )
...
DQuery := oServer:Query( "Select NUM_SEQ, NRPED, cupom, data_orc, cod_cli, nom_cli, cl_cgc ,rgie, cl_end, cl_pessoa, cl_cid, cod_ibge, ed_numero, email, cep, bairro, estado, desconto,  valor_tot, total_ven, desc1, desc2 From ORCAMENT WHERE NUM_SEQ = "+chave+" Order By emissao" )
If DQuery:NetErr()
  	MsgStop(DQuery:Error())
...
IF nfe.oRad3.VALUE == 1
eQuery := oServer:Query( "Select CbdcProd ,CbdcEAN,CbdxProd,CbdnItem,CbdvProd,CbdNtfNumero,CbdNCM,CbdqCOM,CbdvUnCom,CbdUCom,CbdvDesc,cbdvoutro_item,CbdvAliq,CbdCFOP,cbdcsittrib From itemNFCE WHERE CbdNtfNumero ="+ntrim(xnum)+" and CbdNtfSerie = "+alltrim(Txt_SERIE)+" Order By CbdxProd" )
endif
IF nfe.oRad3.VALUE == 2
 eQuery := oServer:Query( "Select COD_PROD,produto,descricao,itens,subtotal,nseq_orc,ncm,qtd,valor,unid,TOTAL,STATUS,ALIQUOTA,CFOP,stb From ITEMORC WHERE nseq_orc ="+ntrim(xnum)+" Order By descricao" )
endif
...
mgREGIME:=1
 ...
 XValor_DESC:=ITEMNFE->SubTotal*PORCDESCONTO/100
        ...
        nFrete_Item:=0
		 ...
		 aadd(DADOS_NFe,{'Descricao=' +Alltrim(ITEMNFE->DESCRICAO + " (    Tributos  ) " + ALLTRIM(TRANSFORM(ITEMNFE->N_IBPT  ,"99,999,999.99")) ) })
		 aadd(DADOS_NFe,{'EAN=' })
		...
		* aadd(DADOS_NFe,{'ValorDesconto=' +ALLTRIM(TRANSFORM(XValor_DESC,"@ 99999999999999.99999"))  })
...
soma_desconto:=soma_desconto+XValor_DESC
XXTOTALITENS :=XXTOTALITENS+ITEMNFE->SubTotal
...
/*
 ...
 DO WHILE XXX<=10
   IF !FILE("C:\ACBrNFeMonitor\SAI.TXT") 
    MY_WAIT( 3 ) 
    xxx++
   ELSE
  ////RETORNO////
   cFileDanfe:="C:\ACBrNFeMonitor\SAITXT"
   exit
   END
enddo
...
///--->>>capa da nota
		...
		xCbdJustificativa     := XNFE_REFERENCIADA
		XMunEmit_cMun         := alltrim(nfe.txt_nfecc.value)
		xCbdcUF               := "11"
		DIA=DAY(date())
        MES=MONTH(date())
...
******************************
Function EsperaResposta2(cFile)
******************************
