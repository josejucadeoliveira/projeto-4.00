...
/*
fQuery := oServer:Query( "Select MAX(SEQ_BOL)FROM CONTA SEQ_BOL")
...
//CCNumero := substr(alltrim(str(HB_RANDOM(123513,999999))),1,6)
cNumero := substr(alltrim(str(HB_RANDOM(123456,999999))),1,6)
cNumero        := val(cNumero)
 
//publ cNumero:=Numero+1 
abreboleto()
...
@ 040,10 textBTN textBTN_cliente;
	      numeric;
	      width 70;
	      value 200;
	      ...
	      maxlength 4;
	      rightalign;
	      Action {||GetCode_cli(Mostra_linha(),oform_boleto.txt_documento.setfocus)};
	      ON enter {||iif(!Empty(oform_boleto.textBTN_cliente.Value),pesqcli(),oform_boleto.txt_documento.setfocus)};
  ...
  ON KEY F8 ACTION { || GetCode_cli(),mostra_linha()} 
...
////////////////////////
   ...
   @ 80,L1+460  LABEL lvcto ;
	...
	HEIGHT 20;
	WIDTH 90;
	ON enter {||boleto_salva3(),mostra_linha()}
//////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////
   ...
   @ 530, 0200 BUTTON oBut2 CAPTION "&Imprimir";
      WIDTH 0080 HEIGHT 0024;
	 ACTION  (printPV:= .t., boleto_imprime_hum(boleto->numero,printPdf ),guarda_boleto() ) 
  ...
  fQuery:Destroy()	
			...
			boleto_salva2()
			 ...
			 ELSE
					   ...
					   boleto->CLINUMERO      :=oform_boleto.txt_numero.value
...
Local oRow
  ...
  fQuery:= oServer:Query( "Select tipo,cnpj,ie,cpf,rg,razaosoc,endereco,numero, cidade,uf From cliente WHERE codigo = " + AllTrim(pCode))
  If fQuery:NetErr()
...
oform_boleto.textBTN_cliente.value:= xxcodigo
fQuery:Destroy()	
form_AUTO.release
return(pcode)
...
fQuery := oServer:Query( "Select codigo,razaosoc, cnpj, ie ,cpf,rg From cliente WHERE razaosoc LIKE "+cSearch+" Order By razaosoc" )
...
For i := 1 To fQuery:LastRec()
  oRow := fQuery:GetRow(i)
  ADD ITEM { Str(oRow:fieldGet(1), 5), oRow:fieldGet(2), oRow:fieldGet(3), oRow:fieldGet(4), oRow:fieldGet(5), oRow:fieldGet(6) } TO Grid_22 Of form_auto
 fQuery:Skip(1)
  Next
fQuery:Destroy()
...
static FUNCTION boleto_multiimpressao
...
DEFINE WINDOW wbolseq  ; 
   ...
   @ lb,500   BUTTON bconfirma CAPTION 'Continuar' FLAT  WIDTH 70  HEIGHT 20  ;
                 ACTION ( wbolseq_confirma() )

   @ lb,580   BUTTON bcancela CAPTION 'Cancelar' FLAT  WIDTH 70  HEIGHT 20  ;
                 ACTION ( wbolseq.release )
...
//////////////////////////////////////////////
static FUNCTION boleto_imprime_hum(nbn,npn)
/////////////////////////////////////////////
//close all
...
aadd(DADOS_TITULO,{'NumeroDocumento='+ boleto->DOCTO})
...
*xxANO     :=val(xxANO)
*NUMERO    :=val(NUMERO)
resultado:=alltrim(zNUMERO+xxANO)
...
procedure gerar_dv_barra(br_barra)
  Local cCampo1, cCampo2, cCampo3, cCampo4
...
************************************************************************
STATIC Function IBR_COMANDO_BOLETO(CMD,VET_PARAM,ESPERA,TENTA)
