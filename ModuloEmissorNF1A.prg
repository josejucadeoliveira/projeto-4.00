...
DEFINE WINDOW NonStop ;
	...
	@ 10,10 LABEL Lbl_Title VALUE "Aguarde Verificando Serviço (http://www.nfe.fazenda.gov.br/portal)" AUTOSIZE

	@ 30,160 LABEL Lbl_Progress VALUE "" AUTOSIZE
...
NonStop.Lbl_Progress.Value := "Aguande"
...
PUBLIC s_CStat:=sCStat
 if sCStat='107'
      else 
	//COD_RETORNONFE(sXMotivo)
   	  MsgOkCancel("Erro O Serviço Solicitado não Esta Ativo, ou sem Conecção na Internet, o Status serviço; Inativo ou Inoperante tente novamente","Atenção",IDI_COMP,.f.)
*NonStop.Release()
* return(.f.)
endif
...
// loca chave := 0
 ...
 //nfe.textBTN_cfop.value :=5929
  IF EMPTY(DADOSNFE->NUM_SEQ )
   else
   MsgINFO("So um Cupom por nota")  
   return nil
  ENDIF
...
Reconectar_A() 
 ...
 public C_ALIQUOTA:=aRow:fieldGet(1)
         ...
         if empty(NFe.txt_DAV.value)
	           msgexclamation('Digite o Cupom (COO)','Atenção')
...
If DQuery:NetErr()
  ...
  xestado         :=alltrim(dRow:fieldGet(17))
  xDESCONTO       :=dRow:fieldGet(18)
  xCCF            :=dRow:fieldGet(19)
  xSERIE          :=dRow:fieldGet(20)  
  xTOTAL_VEN      :=(dRow:fieldGet(21))
  xVALOR_TOT      :=(dRow:fieldGet(22))
  xdesc1          :=(dRow:fieldGet(23))
  xdesc2          :=(dRow:fieldGet(24)) 
//  msginfo(xestado)
...
IF nfe.oRad3.VALUE == 1
eQuery := oServer:Query( "Select COD_PROD,produto,descricao,itens,subtotal,nseq_orc,ncm,qtd,valor,serie,unid ,status From ITEMCUP WHERE nseq_orc ="+ntrim(xnum)+" Order By descricao" )
endif
IF nfe.oRad3.VALUE == 2
...
xCancelado   :=eRow:fieldGet(12)
   ...
   // XSUBTOTAL       := Form_PDV.Text_quant.value*val(Form_PDV.valor_unit.VALUE)
...
xVALOR_TOT:=int(xVALOR_TOT*100)/100
 ...
 NFE.Txt_desconto.value     :=DADOSNFE->desc2      //XVALOR_DESC
...
eemail         :=ALLTRIM(nfe.Txt_email.value)
//eemail:="medial.comercio@yahoo.com.br"
